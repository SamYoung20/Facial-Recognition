...
size(test_images);
T = test_images(50:225,50:200, 1);
test_images_smFace= test_images(50:225,50:200,:);
size(test_images_smFace)
axis equal

%% 2. Reshape into column vectors for each image.
facesStacked = reshape(test_images_smFace ,size(test_images_smFace,1)*size(test_images_smFace,2),size(test_images_smFace,3));
...
meanFace = sumFace./size(test_images_smFace,3);

%% 4. Test Mean Face
meanTest = reshape(meanFace,size(test_images_smFace,1),size(test_images_smFace,2));
axis equal
...
Ui;
testU = reshape(Ui(:,2),size(test_images_smFace,1), size(test_images_smFace,2)); % the largest eigenvalue's corresponding eigenface normalized
...
reshRandomsmFace = faces_test_hard(50:225,50:200, idx);

%reshape image to column vector
randomsmFace = reshape(faces_test_hard(50:225,50:200,idx),size(faces_test_easy(50:225,50:200,idx),1)*size(faces_test_easy(50:225,50:200,idx),2),1);
%center data on mean
centeredRandomFace = randomsmFace - meanFace;
randomsmFace;
...
subplot(2,1,1), imagesc(reshRandomsmFace); colormap('gray');
axis equal
xlim([0 256]);
ylim([0 256]);
